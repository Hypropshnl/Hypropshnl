    ...
    public function handle()
    {
        ...
        foreach ($VMSchedule as $schedule){
            ...
            $lastVehicleOdometerRecord = VehicleOdometerLog::where('status', '=',Utility::STATUS_ACTIVE)
                ...
                if($mileageDuringSchedule != 0 && $schedule->reminder->mileage != 0 && $accumulatedMileage >= $schedule->reminder->mileage){
                    ...
                    $newStartMileage = $mileageDuringSchedule + $accumulatedMileage;
                    VehicleMaintenanceSchedule::defaultUpdate('id',$schedule->id,['mileage'=>$newStartMileage]);
                }
                ...
                if(!empty($schedule->reminder->last_reminder)){
                    $dateTime1 = date("Y-m-d", strtotime(Carbon::parse($schedule->reminder->last_reminder)->subDays(2)));
                    $dateTime2 = date("Y-m-d", strtotime(Carbon::parse($schedule->reminder->last_reminder)->subDays(1)));
            ...
            if(!empty($schedule->reminder->interval) && $schedule->reminder->interval != 0) {
                $dateTime1 = date("Y-m-d", strtotime(Carbon::parse($schedule->reminder->next_reminder)->subDays(2)));
                $dateTime2 = date("Y-m-d", strtotime(Carbon::parse($schedule->reminder->next_reminder)->subDays(1)));
