...
use App\Mail\RFQBidReportMail;
use App\Mail\VendorQuoteProposalMail;
...
class Notify
{
    ...
    //SEND VENDOR REGISTER MAIL
    public static function vendorRegisterMail($viewPage,$arrayContent = [],$email,$fullName = '',$subject = ''){
        
        //Mail::to($email)->send(new VendorRegisterMail($arrayContent));
    }
    ...
    public static function leaveRequestMailDispatch($viewPage,$objContent,$email,$fullName ='',$subject = ''){
    ...
    public static function JobApplicantMail($viewPage,$objContent,$email,$fullName ='',$subject = ''){

        //Mail::to($email)->send(new JobApplicantMail($objContent));
    }

    public static function materialRequestMail($viewPage,$objContent,$email,$fullName ='',$subject = ''){

        //Mail::to($email)->send(new MaterialRequestMail($objContent));
    }
    ...
    public static function VendorQuoteProposalMail($viewPage,$objContent,$email,$fullName ='',$subject = ''){

        //Mail::to($email)->send(new VendorQuoteProposalMail($objContent));
    }

    public static function rfqBidReportMail($viewPage,$objContent,$email,$fullName ='',$subject = ''){

        //Mail::to($email)->send(new RFQBidReportMail($objContent));
    }
    ...
    public static function BirthdayMail($viewPage,$objContent,$email,$fullName ='',$subject = ''){

        //Mail::to($email)->send(new BirthdayMail($objContent));
        //dispatch(new SendGeneralEmail($email,$objContent));
    }

    public static function AdminMail($viewPage,$arrayContent = [],$email,$fullName = '',$subject = ''){

        //Mail::to($email)->send(new AdminRequestMail($arrayContent));
        //dispatch(new SendAdminRequestMail($email,$arrayContent));
    }
    ...
    public static function VehicleMaintenanceScheduleReminder($vehicle,$serviceArr)
    {
        ...
        foreach ($fleetManagers as $userData) {
            ...
            $messageBody = "Hello " . $userData->access_user->firstname . ", the vehicle " . $vehicle->make->make_name
