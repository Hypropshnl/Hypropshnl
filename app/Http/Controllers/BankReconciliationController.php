...
class BankReconciliationController extends Controller
{
    ...
    public function createReconciliation(Request $request)
    {
        ...
        if($countClearedDeposits > 0 || $countClearedPayments > 0){
                ...
                $dbDATA = [
                    ...
                    'register_balance' => $accountData->virtual_balance,
        ...
        if($validator->passes()){
            ...
            if(in_array($categoryId,Utility::BALANCE_SHEET_ACCOUNTS) && !empty($finYear)){
                ...
                if(!empty($lastReconciledData)){
                    ...
                    $reconcilePayments = AccountJournal::specialColumnsDate33('reconcile_status',Finance::uncleared,'chart_id',$accountId,'debit_credit',$payment,$dateArray);
                    $reconcileDeposits = AccountJournal::specialColumnsDate33('reconcile_status',Finance::uncleared,'chart_id',$accountId,'debit_credit',$deposit,$dateArray);
    ...
    public function fetchBeginingBalance(Request $request)
    {
        ...
        $lastReconciledData = BankReconciliation::firstRow('account_id',$accountId);
        $beginingData = AccountJournal::firstRowAsc('chart_id',$accountId);
        $beginingBalance = (!empty($beginingData)) ? $beginingData->trans_total : 0.00;    $endingBalance = 0.00;
        $deposit = ''; $payment = '';   $balanceChanges = 0.00;
        $currReconciledAmount = 0.00;   $reconciledAmount = 0.00;
    ...
    public function singleReport(Request $request, $id)
    {
        //
        {
            ...
            $reconcilePayments = AccountJournal::specialColumnsDate44('chart_id',$accountId,'debit_credit',$payment, 'reconcile_id',$data->id,$dateArray);
            $reconcileDeposits = AccountJournal::specialColumnsDate44('chart_id',$accountId,'debit_credit',$deposit, 'reconcile_id',$data->id,$dateArray);
    ...
    public function redoBankReconciliation(Request $request, $id){
        ...
        $reconcilePayments = AccountJournal::specialColumnsOrDate44('chart_id',$accountId,'debit_credit',$payment, 'reconcile_id',$data->id,'reconcile_status',Finance::uncleared,$dateArray);
        $reconcileDeposits = AccountJournal::specialColumnsOrDate44('chart_id',$accountId,'debit_credit',$deposit, 'reconcile_id',$data->id,'reconcile_status',Finance::uncleared,$dateArray);
