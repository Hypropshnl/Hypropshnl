...
class User extends Authenticatable
{
    ...
    public static $mainRules = [
        ...
        'role' => 'required',
        'firstname' => 'required',
        'photo' => 'sometimes|mimes:jpeg,JPEG,jpg,JPG,png,bmp,gif|max:2048',
        'sign' => 'sometimes|mimes:jpeg,JPEG,jpg,JPG,png,bmp,gif|max:2048',
        'lastname' => 'required',
    ...
    public static function getAllDataDate($date1, $date2)
    {
    ...
    public static function specialColumnsDate($column, $post, $date1, $date2)
    {
        //Utility::specialColumns(self::table(),$column, $post);
    ...
    public static function massDataConditionDate($column, $post, $date1, $date2)
    {
        return static::where('status', '=',Utility::STATUS_ACTIVE)->whereIn($column,$post)
        ->whereDate('created_at', '>=', $date1)->whereDate('created_at', '<=', $date2)->orderBy('id','DESC')->get();
    ...
    public static function massDataConditionDate2($column, $post, $column2, $post2, $date1, $date2)
    {
        return static::where('status', '=',Utility::STATUS_ACTIVE)->whereIn($column,$post)->where($column2,$post2)
        ->whereDate('created_at', '>=', $date1)->whereDate('created_at', '<=', $date2)->orderBy('id','DESC')->get();
    ...
    public static function massDataMassOrCondition($column, $post, $column2, $post2)
    {
        //return Utility::massDataCondition(self::table(),$column, $post, $column2, $post2);
        return static::where('status', '=',Utility::STATUS_ACTIVE)->where('active_status', '=',Utility::STATUS_ACTIVE)
        ->where(function ($query) use($column, $post, $column2, $post2){
            $query->whereIn($column,$post)->orWhereIn($column2,$post2);
       })->orderBy('id','DESC')->get();
    ...
    public static function searchUser($value){
        return static::leftjoin('department', 'department.id', '=', 'users.dept_id')
            ...
            })->orderBy('users.id','DESC')->get(['users.uid']);
    }
        ...
        return static::leftjoin('department', 'department.id', '=', 'users.dept_id')
            ...
            ->where(function ($query) use($value){
                $query->where('users.lastname','LIKE','%'.$value.'%')
                    ...
                    ->orWhere('salary.salary_name','LIKE','%'.$value.'%')->orWhere('position.position_name','LIKE','%'.$value.'%')
                    ->orWhere('department.dept_name','LIKE','%'.$value.'%')->orWhere('roles.role_name','LIKE','%'.$value.'%');
            })->orderBy('users.id','DESC')->get(['users.firstname','users.lastname','users.othername','users.email','users.id']);
    }

    public static function searchEnabledUserAll($value){
        return static::leftjoin('department', 'department.id', '=', 'users.dept_id')
            ->leftjoin('salary', 'salary.id', '=', 'users.salary_id')
            ->leftjoin('position', 'position.id', '=', 'users.position_id')
            ->leftjoin('roles', 'roles.id', '=', 'users.role')
