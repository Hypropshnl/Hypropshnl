...
class BehavComp extends Model
{
    ...
    public static $mainRules = [
        'core_comp.*' => 'required',
        'element.*' => 'required',
        'behav_level.*' => 'required',
    ];

    public static $mainRulesEdit = [
        'core_comp.*' => 'required',
        'element.*' => 'required_with:core_comp.*',
        'behav_level.*' => 'required_with:core_comp.*',
    ];

    public function behavScores(){
        return $this->hasMany('App\model\AppraisalBehavReviewerScores','behav_id','id')->where('status',Utility::STATUS_ACTIVE)->orderBy('id','ASC');
