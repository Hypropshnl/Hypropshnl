...
class CompetencyAssess extends Model
{
    ...
    public static $mainRulesEdit = [
        'core_comp.*' => 'required',
        'capable.*' => 'required_with:core_comp.*',
        'level.*' => 'required_with:core_comp.*',
    ];

    public function techScores(){
        return $this->hasMany('App\model\AppraisalTechReviewerScores','tech_id','id')->where('status',Utility::STATUS_ACTIVE)->orderBy('id','ASC');
