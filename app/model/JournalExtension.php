...
class JournalExtension extends Model
{
    ...
    public static function specialColumnsDate($column, $post,$date)
    {
        ...
        return static::where('status', '=',Utility::STATUS_ACTIVE)->whereIn($column,$post)
            ...
            ->get(['balance','balance_trans','journal_status','due_date','id','trans_total','sum_total','trans_curr','transaction_type','cash_status','post_date','status','vendor_customer','class_id','location_id','employee_id','file_no','created_by','updated_by','created_at','updated_at']);
    ...
    public static function specialColumnsDateTransaction($column, $post,$dateArray)
    {
        //return Utility::specialColumns2(self::table(),$column, $post, $column2, $post2);
        return static::where('status', '=',Utility::STATUS_ACTIVE)->whereIn($column,$post)
            ->whereDate('post_date', '>=', $dateArray[0])->whereDate('post_date', '<=', $dateArray[1])->orderBy('id','DESC')
