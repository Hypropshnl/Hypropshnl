...
class RFQExtension extends Model
{
    ...
    public function rfqDetail(){
        return $this->hasMany('App\model\RFQ','rfq_id','id')->where('status', Utility::STATUS_ACTIVE);
    ...
    public function dataStatus(){
        return $this->belongsTo('App\model\AccountStatus','rfq_status','id')->withDefault();

    }

    public function rfqReport(){
        return $this->belongsTo('App\model\RFQBidReport','id','rfq_id')->withDefault();

    }

    public static function paginateAllData()
    {
        return static::where('status', '=',Utility::STATUS_ACTIVE)->orderBy('id','DESC')->paginate('35');
    ...
    public static function specialColumnsDatePage2($column, $post, $column2, $post2, $column3, $post3)
    {
        return static::where('status', '=',Utility::STATUS_ACTIVE)->where($column, '>=',$post)
            ->where($column2, '=',$post2)->where($column3, '=',$post3)->orderBy('id','DESC')->paginate(Utility::P100);
