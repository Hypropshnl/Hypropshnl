...
class UnitGoal extends Model
{
    ...
    public function unit_goal_ext(){
        return $this->hasMany('App\model\UnitGoalExt','unit_goal_id','id')->where('status',Utility::STATUS_ACTIVE)->orderBy('id','ASC');
    ...
    {
		return static::where('status', '=',Utility::STATUS_ACTIVE)->where($column, '=',$post)
		->where($column2, '=',$post2)->orderBy('id','DESC')->avg($avgColumn);
    ...
    {
		return static::where('status', '=',Utility::STATUS_ACTIVE)->where($column, '=',$post)
		->where($column2, '=',$post2)->where($column3, '=',$post3)->orderBy('id','DESC')->avg($avgColumn);
    ...
    public static function getUniqueUser3($column, $post, $column2, $post2, $column3, $post3)
    {
        return self::select('user_id', DB::raw('COUNT(*) as total'))
        ->where('status', '=',Utility::STATUS_ACTIVE)
        ->where($column, '=',$post)->where($column2, '=',$post2)->where($column3, '=',$post3)
        ->groupBy('user_id');
