...
class VehicleFuelLog extends Model
{
    ...
    public static function massDataDate3($column, $post, $dateArray)
    {
        ...
        return static::where('status', '=',Utility::STATUS_ACTIVE)->whereIn($column, $post)
            //->whereBetween('purchase_date',$dateArray)
            ->whereDate('purchase_date', '>=', $dateArray[0])->whereDate('purchase_date', '<=', $dateArray[1])
            ->orderBy('id','DESC')->get();
    ...
    {
        return static::where('status', '=',Utility::STATUS_ACTIVE)->where($column,$post)
            ->where($column2,$post2)->whereIn($column3,$post3)
            ->whereDate('purchase_date', '>=', $dateArray[0])->whereDate('purchase_date', '<=', $dateArray[1])
            ->orderBy('id','DESC')->get();
    ...
    {
        return static::where('status', '=',Utility::STATUS_ACTIVE)->where($column,$post)
            ->where($column2,$post2)//->whereBetween('purchase_date',$dateArray)
            ->whereDate('purchase_date', '>=', $dateArray[0])->whereDate('purchase_date', '<=', $dateArray[1])
            ->orderBy('id','DESC')->get();
