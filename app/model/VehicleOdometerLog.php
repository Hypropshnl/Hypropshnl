...
class VehicleOdometerLog extends Model
{
    ...
    public static function specialColumnsDate($dateArray)
    {
        return static::where('status', '=',Utility::STATUS_ACTIVE)
            //->whereBetween('log_date',$dateArray)
            ->whereDate('log_date', '>=', $dateArray[0])->whereDate('log_date', '<=', $dateArray[1])
            ->orderBy('id','DESC')->get();
    }
    ...
    {
        return static::where('status', '=',Utility::STATUS_ACTIVE)->where($column,$post)
            ->where($column2,$post2)//->whereBetween('log_date',$dateArray)
            ->whereDate('log_date', '>=', $dateArray[0])->whereDate('log_date', '<=', $dateArray[1])
            ->orderBy('id','DESC')->get();
