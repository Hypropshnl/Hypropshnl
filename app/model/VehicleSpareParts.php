...
class VehicleSpareParts extends Model
{
    //
    protected  $table = 'vehicle_spare_parts';

    private static function table(){
        return 'vehicle_spare_parts';
    }
    /**
    ...
    public static function massDataDate3($column, $post, $dateArray)
    {
        ...
        return static::where('status', '=',Utility::STATUS_ACTIVE)->whereIn($column, $post)
            //->whereBetween('service_date',$dateArray)
            ->whereDate('service_date', '>=', $dateArray[0])->whereDate('service_date', '<=', $dateArray[1])
            ->orderBy('id','DESC')->get();
    ...
    {
        return static::where('status', '=',Utility::STATUS_ACTIVE)->where($column,$post)
            ->where($column2,$post2)//->whereBetween('service_date',$dateArray)
            ->whereDate('service_date', '>=', $dateArray[0])->whereDate('service_date', '<=', $dateArray[1])
            ->orderBy('id','DESC')->get();
    ...
    {
        return self::whereIn($column, $post)->where('status', Utility::STATUS_ACTIVE)
            ->orderBy('id','DESC')->get();
    ...
    public static function searchVehicleSpareParts($value){
        return static::where('vehicle_spare_parts.status', '=',Utility::STATUS_ACTIVE)
            ->where(function ($query) use($value){
                $query->Where('vehicle_spare_parts.name','LIKE','%'.$value.'%') 
                    ->orWhere('vehicle_spare_parts.part_no','LIKE','%'.$value.'%');
            })->get();
    }
