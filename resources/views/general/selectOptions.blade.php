...
@if($type == 'search_inventory')
    @foreach($optionArray as $data)
        ...
        <li>
            <a  onclick="dropdownItem('{{$searchId}}','{{$data->item_name}}','{{$hiddenId}}','{{$data->id}}','{{$listId}}');">{{$data->item_name}} ({{$data->item_no}})</a>
        </li>
...
@if($type == 'search_inventory_transact')
    @foreach($optionArray as $data)
        @php $itemDesc = ($billInvoice == Utility::PURCHASE_DESC) ? $data->purchase_desc : $data->sales_desc; @endphp
        <li>
            <a  onclick="dropdownItemInv('{{$searchId}}','{{$data->item_name}}','{{$hiddenId}}','{{$data->id}}','{{$listId}}','{{$billInvoice}}','<?php echo url('inventory_details') ?>','{{$descId}}','{{$rateId}}','{{$unitMId}}','{{$subTotalId}}','{{$sharedSubTotal}}','{{$overallSum}}','{{$foreignOverallSum}}','<?php echo url('amount_to_default_curr') ?>','{{$qtyId}}','{{$vendCustId}}','{{$postDateId}}','{{$totalTaxId}}');">{{$data->item_name}} ({{$data->item_no}}) - ({{$itemDesc}})</a>
        </li>

    @endforeach
@endif

@if($type == 'search_inventory_detail')
    @foreach($optionArray as $data)
        @php $itemDesc = ($billInvoice == Utility::PURCHASE_DESC) ? $data->purchase_desc : $data->sales_desc; @endphp
        <li>
            <a  onclick="dropdownItemInvDetail('{{$searchId}}','{{$data->item_name}}','{{$hiddenId}}','{{$data->id}}','{{$listId}}','{{$billInvoice}}','<?php echo url('inventory_details') ?>','{{$descId}}','{{$rateId}}','{{$unitMId}}','{{$qtyId}}');">{{$data->item_name}} ({{$data->item_no}}) - ({{$itemDesc}})</a>
        </li>
...
@if($type == 'core_behav_comp')
        ...
        <select name="{{$param}}"  class="form-control element element_edit" id=""  >
            @if(count($optionArray) > 0)
            @foreach($optionArray as $data)
...
@if($type == 'core_tech_comp')
        ...
        <select name="{{$param}}"  class="form-control capable capable_edit" id=""  >
            @if(count($optionArray) > 0)
            @foreach($optionArray as $data)
                <option value="{{$data->item_desc}}">{{$data->item_desc}}</option>
            @endforeach
...
@if($type == 'w_zones_fetch_extra')

    <select name="{{$zoneId}}"  class="form-control zones" id="{{$zoneId}}" name="{{$zoneId}}" onchange="fillNextInputParamGetVal('{{$zoneId}}','{{$binId}}','<?php echo url('default_select'); ?>','z_bins_extra','{{$warehouseId}}')" >
        @if(count($optionArray) > 0)
            <option value="">Select</option>
            @foreach($optionArray as $data)
                <option value="{{$data->zone_id}}">{{$data->zone->name}}</option>
...
@if($type == 'z_bins_extra')

    <select name="{{$binDisplay}}"  class="form-control bins" id=""  >
        @if(count($optionArray) > 0)
            <option value="">Select</option>
            @foreach($optionArray as $data)
                <option value="{{$data->bin_id}}">{{$data->bin->code}}</option>
...
@if($type == 'search_mr_select')
    @foreach($optionArray as $data)

        <li>
            <a href="#" onclick="dropdownItem('{{$searchId}}','{{$data->mr_number}}','{{$hiddenId}}','{{$data->id}}','{{$listId}}');">{{$data->mr_number}}</a>
        </li>

    @endforeach
@endif

@if($type == 'search_my_mr_select')
    @foreach($optionArray as $data)
...
@if($type == 'crm_stage_search')
    @foreach($optionArray as $data)
        ...
        <li>
            <a  onclick="dropdownItem('{{$searchId}}','{{$data->name}}','{{$hiddenId}}','{{$data->id}}','{{$listId}}');">{{$data->name}} (Stage {{$data->stage}}) ({{$data->probability}}%)</a>
        </li>
...
@if($type == 'search_vehicle_param_repair')
    @foreach($optionArray as $data)

        <li>
            <a  onclick="dropdownItemRep('{{$searchId}}','{{$data->make->make_name}} {{$data->model->model_name}} ({{$data->license_plate}})','{{$hiddenId}}','{{$data->id}}','{{$listId}}','{{$selectDisplayId}}','{{$moduleType2}}','{{$newInputPage}}');">{{$data->make->make_name}} {{$data->model->model_name}} ({{$data->license_plate}})</a>
        </li>

    @endforeach
@endif

@if($type == 'select_vehicle_repair')

    <select name="fault_description"  class="form-control " id=""  >
        @if(count($optionArray) > 0)
            <option value="">Select Fault Description</option>
            @foreach($optionArray as $data)
                <option value="{{$data->id}}">{{$data->fault_desc}}</option>
            @endforeach

        @else
            <option value="">No Fault Description Found</option>
        @endif
    </select>
@endif

@if($type == 'default_search_job')
@foreach($optionArray as $data)

<li>
    <a  onclick="dropdownItem('{{$searchId}}','{{$data->job_title}}','{{$hiddenId}}','{{$data->id}}','{{$listId}}');">{{$data->job_title}}</a>
</li>

@endforeach
@endif

@if($type == 'search_multiple_vehicle_spare_parts_detail')
    @foreach($optionArray as $data)

        <li>
            <a  onclick="dropdownItemMultipleDetail('{{$searchId}}','{{$data->name}} ({{$data->part_no}}) License Plate({{$data->vehicleDetail->license_plate}})','{{$hiddenId}}','{{$data->id}}','{{$listId}}','{{$detailType}}','<?php echo url('fetch_multi_details') ?>','{{$descId}}','{{$itemArrayId}}');">{{$data->name}} ({{$data->part_no}})  License Plate({{$data->vehicleDetail->license_plate}})</a>
        </li>

    @endforeach
@endif

@if($type == 'search_multiple_vendor_detail')
    @foreach($optionArray as $data)

        <li>
            <a  onclick="dropdownItemMultipleDetail('{{$searchId}}','{{$data->name}} ({{$data->email1}})','{{$hiddenId}}','{{$data->id}}','{{$listId}}','{{$detailType}}','<?php echo url('fetch_multi_details') ?>','{{$descId}}','{{$itemArrayId}}');">{{$data->name}} ({{$data->email1}})</a>
        </li>
