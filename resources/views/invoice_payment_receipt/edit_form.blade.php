<form name="" id="editMainForm" onsubmit="false;" class="form form-horizontal" method="post" enctype="multipart/form-data">
        ...
        <div class="row clearfix">
            ...
            <div class="col-sm-4">
                <div class="form-group">
                    ...
                    <div class="form-line">
                        <input type="text" disabled class="form-control" value="" id="sum_total" name="amount_received" placeholder="Amount Received">
                    
                        <input type="hidden" value="0.00" id="sum_total_hidden" name="total_amount" placeholder="Amount Received">
                    </div>
                </div>
        ...
        <div class="row clearfix">
            ...
            <table class="table table-bordered table-hover table-striped" id="account_main_table_edit">
                ...
                <tr>
                    ...
                    <th>Customer</th>
                    <th class="">Open Balance </th>
                    <th class="">Credit </th>
                    <th class="">Credit Applied </th>
                    <th class="">Payment </th>
                        ...
                        <tr id="itemId{{$po->id}}">
                            ...
                            <td>
                                <div class="col-sm-4">
                                    <div class="form-group">
                                        <div class="form-line">
                                            <select  name="credit_applied{{$num1}}" id="credit_applied{{$po->id}}"
                                                onchange="DebitCreditMemoApply('credit_applied{{$po->id}}','credit_applied_class{{$po->id}}','default_credit{{$po->vendor_customer}}','curr_credit{{$po->vendor_customer}}','payment{{$po->id}}','default_total{{$po->id}}','curr_total{{$po->id}}','curr_total_hidden{{$po->id}}','sum_total','sum_total_hidden','total_amount', 'credit_applied_value{{$po->id}}', 'credit_applied_id{{$po->id}}')" >
                                                <option value="">Select Credit Memo</option>
                                                @foreach($po->customerCreditMemoList as $val)
                                                @php $memoFileNo = (!empty($val->file_no)) ? $val->file_no : $val->id; @endphp
                                                <option value="{{$val->balance_trans}}|{{$val->id}}">
                                                    ({{$val->vendorCon->name}} (file No({{$memoFileNo}}))) {{$val->currency->code}}{{$val->balance_trans}}
                                                </option>
                                            @endforeach
                                            </select>
                                            <input type="hidden" id="credit_applied_value{{$po->id}}" class="credit_applied_class{{$po->id}}" name="{{$po->vendor_customer}}credit_applied_value{{$num1}}" value="0.00" class="" />
                                            <input type="hidden" id="credit_applied_id{{$po->id}}" name="{{$po->vendor_customer}}credit_applied_id{{$num1}}" value="" class="" />
                                        </div>
                                    </div>
                                </div>
                            </td>
                            ...
                            <td>         
                                <div class="col-sm-4">
                                    <div class="form-group">
                                        <div class="form-line">
                                            <input type="number" name="payment{{$num1}}" id="payment{{$po->id}}" class="total_amount"
                            ...
                            <td>         
                                ...
                                <input type="hidden"  name="curr_total{{$num1}}" id="curr_total_hidden{{$po->id}}" />
                            </td>
    ...
    <input type="hidden" name="transaction_type" value="{{Finance::invoicePaymentReceipt}}" >
    <input type="hidden" name="credit_transaction_type" value="{{Finance::creditMemo}}" >
</form>
