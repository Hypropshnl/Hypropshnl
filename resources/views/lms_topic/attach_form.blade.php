...
<?php $attach = json_decode($edit->documents,true); $num=0; ?>
@if(empty($edit->documents) || count($attach) < 1)
    No Document
@else
...
<table class="table table-responsive">
    ...
    <tr>
        <td>File{{$num}}</td>
        <td><a target="_blank" href="<?php echo URL::to('download_lms_topic_attachment?file='); ?>{{$at}}">
        ...
        <td>
            ...
            <button type="button"  onclick="submitMediaForm('attachModal','removeAttachForm','<?php echo url('remove_lms_topic_attachment'); ?>','reload_data',
                    '<?php echo url('lms_topic/'.$edit->course_id.'/'.$edit->lesson_id); ?>','<?php echo csrf_token(); ?>')"
