            ...
            <div class="card">
                <div class="header">
                    ...
                    <ul class="header-dropdown m-r--5">
                        @if($appAccess === 1)
                        <li>
                            <button type="button" onclick="RequestApproval('kid_checkbox','reload_data','<?php echo url('my_sales_order_requests'); ?>',
                                    '<?php echo url('approve_sales_order'); ?>','<?php echo csrf_token(); ?>','1');" class="btn btn-success">
                                <i class="fa fa-check-square-o"></i>Approve
                            </button>
                        </li>
                        <li>
                            <button type="button" onclick="RequestApproval('kid_checkbox','reload_data','<?php echo url('my_sales_order_requests'); ?>',
                                    '<?php echo url('approve_sales_order'); ?>','<?php echo csrf_token(); ?>','0');" class="btn btn-danger">
                            ...
                            @if($data->approval_view == 1 && $data->deny_reason == '')
                                @if($data->next_user == Auth::user()->id)
                                    <tr>
                                        ...
                                        <td>
                                            <a style="cursor: pointer;" onclick="fetchHtml('{{$data->id}}','attach_content','attachModal','<?php echo url('edit_sales_attachment_form') ?>','<?php echo csrf_token(); ?>')"><i class="fa fa-pencil-square-o fa-2x"></i></a>
                                        </td>
                                        <td>
                                            <a style="cursor: pointer;" class="btn btn-info" onclick="fetchHtml2('{{$data->id}}','print_preview','printPreviewModal','<?php echo url('sales_print_preview') ?>','<?php echo csrf_token(); ?>','customer')"><i class="fa fa-pencil-square-o"></i>Customer Preview</a>
                                        </td>
                                        <td>
