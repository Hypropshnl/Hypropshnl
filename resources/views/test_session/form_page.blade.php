            ...
            <div class="header">
                <h2>
                   {{$testSession->session_name}}
                    <small>Click on any of the test category name below to view result after submission</small>
                </h2>
            ...
            <div class="body" id="main_table">
                ...
                <ul class="nav nav-tabs tab-nav-right" role="tablist">
                    @foreach($mainData->category as $cat)
                            ...
                            $disabled = ($cat->resultCheck == 1) ? '' : 'disabled';
                            $disabled = ($mainData->showCat == $cat->id ) ? '' : $disabled; echo $disabled;
                            ?>
                                    ">
...
<script>
    function submitTestDefault(formId,submitUrl,reload_id,reloadUrl,token,buttonId){
        ...
        var timer = (window.localStorage.getItem(triggerButton) === null) ? timed : window.localStorage.getItem(triggerButton);
        ...
        ajax.onreadystatechange = function(){
            if(ajax.readyState == 4 && ajax.status == 200) {
                ...
                if(message2 == 'fail'){
                    ...
                    if(window.localStorage.getItem(triggerButton) != null){
                        clearInterval(timer);
                        var trigger = $("#"+triggerButton);
                        window.localStorage.removeItem(triggerButton);
                    }
    ...
    function startTimer(duration, display,triggerB) {
        ...
        setInterval(function () {
            ...
            //timer--
            var triggerT = $("#"+triggerB);
            if (timer < 1 && timer == 0) {
                //timer = duration;
                clearInterval(timer);
                triggerT.trigger('click');
                triggerT.attr("disabled", true);
